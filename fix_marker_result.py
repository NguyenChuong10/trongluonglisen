...
# We want to replace from `marker_result = await self.zalo_page.evaluate(r"""`
# to `""", {"trackingNumber": tracking_number, "skipScrollSearch": skip_scroll_search})`
...
pattern = re.compile(r'marker_result = await self\.zalo_page\.evaluate\(r"""\n            async \(\{ trackingNumber, skipScrollSearch \}\) => \{.*?""", \{"trackingNumber": tracking_number, "skipScrollSearch": skip_scroll_search\}\)', re.DOTALL)

replacement = f'''marker_result = await self.zalo_page.evaluate(r"""
