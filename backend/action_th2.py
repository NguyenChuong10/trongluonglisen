...
async def run_th2(automation, progress_callback=None, start_tracking=None):
    """TH2: Scans Zalo messages in UP direction, starting from start_tracking anchor, downloading and liking each package one-by-one."""
    direction = "up"
    ...
    if progress_callback:
        await progress_callback("Đang tìm kiếm nhóm 'HÀNG QUÁ TRỌNG LƯỢNG' (TH2)...")
    ...
    try:
        ...
        print(f"[TH2] Lỗi tự động chọn nhóm: {e}")
    ...
    if start_tracking:
        ...
        if found:
            ...
            print(f"[TH2] Không tìm thấy mã mốc qua tìm kiếm Zalo. Thử cuộn lên để tìm mốc...")
            if progress_callback:
            ...
            """, start_tracking)
            if found_fallback:
                print(f"[TH2] Đã định vị thành công mã mốc {start_tracking} bằng cuộn dự phòng.")
            else:
                print(f"[TH2] Không tìm thấy mã mốc {start_tracking} trong DOM. Bắt đầu từ vị trí hiện tại.")
    ...
    if os.path.exists(scanned_path):
        try:
            ...
            print(f"[TH2] Lỗi đọc danh sách cũ: {e}")
    ...
    for step in range(500):
        if automation.stop_requested:
            print("[TH2] Dừng quét theo yêu cầu của người dùng.")
            if progress_callback:
        ...
        if stop_scan_now:
            print("[TH2] Dừng quét do đã gặp mốc tin nhắn cũ.")
            if progress_callback:
        ...
        # 2. Sort results (Scrolling UP: Process from newest to oldest)
        ...
        for item in results:
            ...
            if item["isLiked"]:
                ...
                if consecutive_liked >= 3:
                    print(f"[TH2] Đã gặp 3 đơn liên tiếp ({trk}) đã được like từ trước. Dừng quét!")
                    stop_scan_now = True
                    break
                continue
            else:
        ...
        if target_item:
            ...
            print(f"[TH2] ▶ Phát hiện đơn chưa xử lý: {trk}")
            if progress_callback:
            ...
            try:
                ...
                if len(imgs) >= 1 and len(vids) >= 1:
                    ...
                    processed_trackings.add(trk)
                    download_ok = False
                    print(f"[TH2] ❌ Tải lỗi/thiếu file cho đơn {trk}: Có {len(imgs)} ảnh, {len(vids)} video")
                ...
                try:
                    ...
                    print(f"[TH2] Lỗi ghi file scanned_packages.json: {we}")

            except Exception as dl_err:
                print(f"[TH2] ❌ Lỗi tải đơn {trk}: {dl_err}")
                processed_trackings.add(trk)
                ...
                try:
                    ...
                    if like_res and like_res.get("success"):
                        if like_res.get("alreadyLiked"):
                            print(f"[TH2] Đơn {trk} đã có tim.")
                        else:
                            print(f"[TH2] ❤️ Đã thả tim đơn {trk}")
                    else:
                        print(f"[TH2] ⚠️ Không tim được đơn {trk}: {like_res.get('error') if like_res else 'unknown'}")
                    
                    await asyncio.sleep(0.4)
                except Exception as le:
                    print(f"[TH2] Lỗi thả tim đơn {trk}: {le}")

            # Step C: Scroll a small amount in specified direction after processing each single bill
            scroll_amount_single = 300
            print(f"[TH2] Xử lý xong đơn {trk} → Cuộn lên {scroll_amount_single}px")
            try:
                await automation.zalo_page.evaluate("""
                    (args) => {
                        const amt = args.amt;
                ...
                print(f"[TH2] Lỗi cuộn sau khi xử lý đơn {trk}: {se}")
        ...
        if not sinfo:
            print("[TH2] Không tìm thấy khung cuộn tin nhắn Zalo.")
            break
        ...
        if at_boundary:
            if cur_height == last_scroll_height and cur_top == last_scroll_top:
                limit_retry_count += 1
                print(f"[TH2] Chờ Zalo load thêm tin ({limit_retry_count}/{max_limit_retries})...")
                if progress_callback:
                    await progress_callback(f"Chờ tải thêm tin ({limit_retry_count}/{max_limit_retries})...")
                if limit_retry_count >= max_limit_retries:
                    print("[TH2] Đã đạt biên giới chat và không còn tin nhắn mới. Hoàn thành quét.")
                    break
        ...
        scroll_amount = 500
        print(f"[TH2] Không có đơn chưa xử lý trong view → cuộn lên {scroll_amount}px")
        if progress_callback:
