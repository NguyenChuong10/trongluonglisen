...
async def run_th1(automation, progress_callback=None, start_tracking=None):
    """TH1: Scans Zalo messages in DOWN direction, starting from start_tracking anchor, downloading and liking each package one-by-one."""
    direction = "down"
    ...
    if progress_callback:
        await progress_callback("Đang tìm kiếm nhóm 'HÀNG QUÁ TRỌNG LƯỢNG' (TH1)...")
    ...
    try:
        ...
        print(f"[TH1] Lỗi tự động chọn nhóm: {e}")
    ...
    if start_tracking:
        ...
        if found:
            ...
            print(f"[TH1] Không tìm thấy mã mốc qua tìm kiếm Zalo. Thử cuộn lên để tìm mốc...")
            if progress_callback:
            ...
            """, start_tracking)
            if found_fallback:
                print(f"[TH1] Đã định vị thành công mã mốc {start_tracking} bằng cuộn dự phòng.")
            else:
                print(f"[TH1] Không tìm thấy mã mốc {start_tracking} trong DOM. Bắt đầu từ vị trí hiện tại.")
    ...
    if os.path.exists(scanned_path):
        try:
            ...
            print(f"[TH1] Lỗi đọc danh sách cũ: {e}")
    ...
    for step in range(500):
        if automation.stop_requested:
            print("[TH1] Dừng quét theo yêu cầu của người dùng.")
            if progress_callback:
        ...
        if stop_scan_now:
            print("[TH1] Dừng quét do đã gặp mốc tin nhắn cũ.")
            if progress_callback:
                await progress_callback("Đã chạm mốc tin nhắn cũ đã xử lý. Hoàn thành!")
            break
        ...
        # 2. Sort results (Scrolling DOWN: Process from oldest to newest)
        ...
        for item in results:
            ...
            if item["isLiked"]:
                ...
                if consecutive_liked >= 3:
                    print(f"[TH1] Đã gặp 3 đơn liên tiếp ({trk}) đã được like từ trước. Dừng quét!")
                    stop_scan_now = True
                    break
                continue
            else:
        ...
        if target_item:
            ...
            print(f"[TH1] ▶ Phát hiện đơn chưa xử lý: {trk}")
            if progress_callback:
            ...
            try:
                ...
                if len(imgs) >= 1 and len(vids) >= 1:
                    ...
                    processed_trackings.add(trk)
                    download_ok = False
                    print(f"[TH1] ❌ Tải lỗi/thiếu file cho đơn {trk}: Có {len(imgs)} ảnh, {len(vids)} video")
                ...
                try:
                    ...
                    print(f"[TH1] Lỗi ghi file scanned_packages.json: {we}")

            except Exception as dl_err:
                print(f"[TH1] ❌ Lỗi tải đơn {trk}: {dl_err}")
                processed_trackings.add(trk)
                ...
                try:
                    ...
                    if like_res and like_res.get("success"):
                        if like_res.get("alreadyLiked"):
                            print(f"[TH1] Đơn {trk} đã có tim.")
                        else:
                            print(f"[TH1] ❤️ Đã thả tim đơn {trk}")
                    else:
                        print(f"[TH1] ⚠️ Không tim được đơn {trk}: {like_res.get('error') if like_res else 'unknown'}")
                    
                    await asyncio.sleep(0.4)
                except Exception as le:
                    print(f"[TH1] Lỗi thả tim đơn {trk}: {le}")

            # Step C: Scroll a small amount in specified direction after processing each single bill
            scroll_amount_single = 300
            print(f"[TH1] Xử lý xong đơn {trk} → Cuộn xuống {scroll_amount_single}px")
            try:
                await automation.zalo_page.evaluate("""
                    (args) => {
                        const amt = args.amt;
                ...
                print(f"[TH1] Lỗi cuộn sau khi xử lý đơn {trk}: {se}")
        ...
        if not sinfo:
            print("[TH1] Không tìm thấy khung cuộn tin nhắn Zalo.")
            break
        ...
        if at_boundary:
            if cur_height == last_scroll_height and cur_top == last_scroll_top:
                limit_retry_count += 1
                print(f"[TH1] Chờ Zalo load thêm tin ({limit_retry_count}/{max_limit_retries})...")
                if progress_callback:
                    await progress_callback(f"Chờ tải thêm tin ({limit_retry_count}/{max_limit_retries})...")
                if limit_retry_count >= max_limit_retries:
                    print("[TH1] Đã đạt biên giới chat và không còn tin nhắn mới. Hoàn thành quét.")
                    break
        ...
        scroll_amount = 500
        print(f"[TH1] Không có đơn chưa xử lý trong view → cuộn xuống {scroll_amount}px")
        if progress_callback:
